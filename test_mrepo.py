...
class Testlinksync(unittest.TestCase):
    def setUp(self):
        ...
        for i in range(4):
            _touch(path_join(srcdir, str(i) + ".rpm"))
        _touch(path_join(srcdir, "dontsync.txt"))

        os.mkdir(path_join(srcdir, "a"))
        _touch(path_join(srcdir, "a", "2.rpm"))
        _touch(path_join(srcdir, "a", "a.rpm"))

        self.localdir = localdir = path_join(config.srcdir, "testdist-i386", "local")
        os.makedirs(localdir)
        for i in ("local.rpm", "dont_sync2.txt"):
            _touch(path_join(localdir, i))
    ...
    def tearDown(self):
        ...
        # for safety-reasons:
        if tmpdir.count("/") < 2:
            raise Exception(f"Will not remove tmpdir {tmpdir}")  # pylint: disable=broad-exception-raised
